 ...
 * Old version can be found here - 
 ...
 Taken from http://www.krishnabharadwaj.info/dynamic-programming/
 (has solution Longest Increasing Subsequence + ZigZag Sequence)
 Compilation: g++ -g -O0 BadNeighbors.cpp -o badNeighbors
  -O0 allows to debug for loops (no vars are optimized out)
*/
...
int BadNeighbors::maxDonations(vector <int> amount){
    ...
    if(i_included[N-1] && zero_included[N-1]){
        return max(max(dp[N-2], dp[N-1] - amount[N-1]),  dp[N-1] - amount[0]);
    } else {
        return dp[N-1];
    }
}
